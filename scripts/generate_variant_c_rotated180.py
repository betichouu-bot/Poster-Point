"""
Regenerate Variant C samples for a list of poster numbers, but rotate the
source poster 180° before splitting/composing. Overwrites the C variant files
 in outputs/SPLIT_POSTERS/samples/.

Usage:
  python scripts/generate_variant_c_rotated180.py --numbers 24,39 --auto-detect
"""
...
parser.add_argument('--numbers', type=str, required=True, help='Comma-separated numbers, e.g. 24,39')
...
targets = []
for n in nums:
    matches = [f for f in all_names if f.startswith(f'1 ({n})')]
    if matches:
...
for name in targets:
    ...
    meta = {
        ...
        'params': {'spacing': spacing, 'scale': args.scale, 'auto_detect': bool(args.auto_detect), 'rotated_source_degrees': 180}
    }
    meta_path = os.path.join(OUT_DIR, f"{base}_C_rows_rotated_columns.json")
    try:
        with open(meta_path, 'w', encoding='utf-8') as mf:
            json.dump(meta, mf, indent=2)
    except Exception as e:
        print('Failed to write metadata for', base, e)

    print('Saved rotated-C sample', out_path)

print('Done. Processed', len(targets), 'items')
